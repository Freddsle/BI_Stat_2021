...
There are `r sum(PercentageFeatureSet(pbmc1, pattern = "^MT-"))` of transcripts mapping to mitochondrial genes.
...
PCA and t-SNE both show 9 cell types. PCA plot shows that two clusters are partially isolated, and other 5 clusters are overlapping. t-SNE plot shows that one cluster is fully isolated, and other groups are partially overlapping. t-SNE separate fully overlapped clusters from PCA.
