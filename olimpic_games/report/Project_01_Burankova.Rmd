...
### Mean and standard deviation of Height variable for female tennis players at the 2000 Games
...
For women tennis players Height variable has **mean = `r sprintf("%.1f", round(mean_woman_2000[1,1,1], 1))`** with **standard deviation = `r sprintf("%.1f", round(sd_woman_2000[1,1,1], 1))`** at the 2000 Olympics.

```{r echo=FALSE, message=FALSE, warning=FALSE}
all_participants %>% 
  filter(Sex == 'F' & !is.na(Height) & Year == 2000 & Sport == 'Tennis') %>%   
  select(ID, Height, Sex, Season, Year, Country_NOC) %>% 
  distinct() %>% 
  ggplot(aes(y = Height, x = Sex)) +
  geom_boxplot(na.rm = TRUE, aes(fill = factor(Sex))) +
...
For all types of medals, the average number of medals per participant per country is `r medals_per_country %>% summarise(mean = mean(medals_per_member)) %>% .[1,1,1] %>% round(., 4)` (standard deviation `r medals_per_country %>% summarise(sd(medals_per_member)) %>% .[1,1,1] %>% round(., 4)`).
...
For Gold medals, the average number of medals per participant per country is `r team_and_gold_medals %>% group_by(Country_NOC) %>% summarize(gold_medals_per_member = round(mean(number_gold_medals / team_members), 3)) %>% summarise(mean(gold_medals_per_member)) %>% .[1,1,1] %>% round(., 4)` (standard deviation `r team_and_gold_medals %>% group_by(Country_NOC) %>% summarize(gold_medals_per_member = round(mean(number_gold_medals / team_members), 3)) %>% summarise(sd(gold_medals_per_member)) %>% .[1,1,1] %>% round(., 4)`).
...
For Silver medals, the average number of medals per participant per country is `r team_and_silver_medals %>% group_by(Country_NOC) %>% summarize(silver_medals_per_member = round(mean(number_silver_medals / team_members), 3)) %>% summarise(mean(silver_medals_per_member)) %>% .[1,1,1] %>% round(., 4)` (standard deviation `r team_and_silver_medals %>% group_by(Country_NOC) %>% summarize(silver_medals_per_member = round(mean(number_silver_medals / team_members), 3)) %>% summarise(sd(silver_medals_per_member)) %>% .[1,1,1] %>% round(., 4)`).
...
For Bronze medals, the average number of medals per participant per country is `r team_and_bronze_medals %>% group_by(Country_NOC) %>% summarize(bronze_medals_per_member = round(mean(number_bronze_medals / team_members), 3)) %>% summarise(mean(bronze_medals_per_member)) %>% .[1,1,1] %>% round(., 4)` (standard deviation `r team_and_bronze_medals %>% group_by(Country_NOC) %>% summarize(bronze_medals_per_member = round(mean(number_bronze_medals / team_members), 3)) %>% summarise(sd(bronze_medals_per_member)) %>% .[1,1,1] %>% round(., 4)`).
...
We have a sufficient number of observations (`r season_number[1,2,1]` for `r season_number[1,1,1]` season and `r season_number[2,2,1]` for `r season_number[2,1,1]`) to consider the distribution of Height variable of female athletes as a normal distribution, which can be seen on the graph.
...
Data about the Olympic Games from  **`r all_participants %>% summarise(min(Year, na.rm=TRUE)) %>% .[1,1,1]` year to `r all_participants %>% summarise(max(Year, na.rm=TRUE)) %>% .[1,1,1]`** year was used. 

Invalid values in the data have been corrected. There are `r nrow(all_participants)` rows and `r ncol(all_participants)` variables in the data.

The Height variable was standardized in two ways, including min-max standardization.

The number of teams (countries) participating in the Olympic Games from `r all_participants %>% summarise(min(Year, na.rm=TRUE)) %>% .[1,1,1]` to `r all_participants %>% summarise(max(Year, na.rm=TRUE)) %>% .[1,1,1]` is **`r team_and_medals %>% group_by(Country_NOC) %>% summarise(n()) %>% summarise(n()) %>% .[1,1,1]`**. Team has an average of **`r round(mean(team_and_medals$team_members), 0)`** participants (with a standard deviation of `r round(sd(team_and_medals$team_members), 2)`) yearly. Each team won an average of **`r round(mean(team_and_medals$number_medals), 2)`** medals of any type (with a standard deviation of `r round(sd(team_and_medals$number_medals), 2)`)  yearly.

Some statistics about the participants in the Olympic Games:
- the youngest athletes at the 1992 Olympics is **`r youngest_men_1992[1,4,1]`** years for men, and **`r youngest_woman_1992[1,4,1]`** years for women.\
- Height variable has:\
  - **mean = `r round(mean_man[1,1,1], 2)`** with **standard deviation = `r round(sd_man[1,1,1], 2)`** for men;\
  - **mean = `r round(mean_woman[1,1,1], 2)`** with **standard deviation = `r round(sd_woman[1,1,1], 2)`** for women .

- for women tennis players Height variable has **mean = `r sprintf("%.1f", round(mean_woman_2000[1,1,1], 1))`** with **standard deviation = `r sprintf("%.1f", round(sd_woman_2000[1,1,1], 1))`** at the 2000 Olympics.\
- the heaviest athlete in the 2006 Olympics participated in the **`r heaviest_2006 %>% select(Sport) %>% .[1,1,1]`** and had weight `r heaviest_2006 %>% select(Weight) %>% .[1,1,1]` kg.\
- women received **`r golds_1980_2010 %>% summarise(n()) %>% .[1,1,1]` gold medals** from 1980 to 2010.\
- John Aalberg participated in Olympic games `r john_games %>% summarise(n()) %>% .[1,1,1]` times.

- Among the participants from 15 to 55 years old:\
  - the smallest group is the `r age_groups_2006 %>% filter(Age_group != '14 and less' & Age_group != '56 and more') %>% group_by(Age_group) %>% summarise(Group_size = n()) %>% filter(Group_size == min(Group_size)) %>% .[1,1,1]` age group;\
  - the most numerous is the `r age_groups_2006 %>% filter(Age_group != '14 and less' & Age_group != '56 and more') %>% group_by(Age_group) %>% summarise(Group_size = n()) %>% filter(Group_size == max(Group_size)) %>% .[1,1,1]` age group.
...
Top 3 countries with the most medals in **Winter Games**:\
- **Gold** medals: `r winter_medals_top[winter_medals_top$Medal == 'Gold',][1,2,1]`, `r winter_medals_top[winter_medals_top$Medal == 'Gold',][2,2,1]` and `r winter_medals_top[winter_medals_top$Medal == 'Gold',][3,2,1]`;\
- **Silver** medals: `r winter_medals_top[winter_medals_top$Medal == 'Silver',][1,2,1]`, `r winter_medals_top[winter_medals_top$Medal == 'Silver',][2,2,1]` and `r winter_medals_top[winter_medals_top$Medal == 'Silver',][3,2,1]`;\
- **Bronze** medals: `r winter_medals_top[winter_medals_top$Medal == 'Bronze',][1,2,1]`, `r winter_medals_top[winter_medals_top$Medal == 'Bronze',][2,2,1]` and `r winter_medals_top[winter_medals_top$Medal == 'Bronze',][3,2,1]`.

Top 3 countries with the most medals in **Summer** **Games**:\
- **Gold** medals: `r summer_medals_top[summer_medals_top$Medal == 'Gold',][1,2,1]`, `r summer_medals_top[summer_medals_top$Medal == 'Gold',][2,2,1]` and `r summer_medals_top[summer_medals_top$Medal == 'Gold',][3,2,1]`;\
- **Silver** medals: `r summer_medals_top[summer_medals_top$Medal == 'Silver',][1,2,1]`, `r summer_medals_top[summer_medals_top$Medal == 'Silver',][2,2,1]` and `r summer_medals_top[summer_medals_top$Medal == 'Silver',][3,2,1]`;\
- **Bronze** medals: `r summer_medals_top[summer_medals_top$Medal == 'Bronze',][1,2,1]`, `r summer_medals_top[summer_medals_top$Medal == 'Bronze',][2,2,1]` and `r summer_medals_top[summer_medals_top$Medal == 'Bronze',][3,2,1]`.

The following hypotheses were subject to and confirmed by statistical tests:\
- there was a significant difference between **the male athletes height compared with female athletes height**;\
- there was a significant difference between **the male athletes weight compared with female athletes weight**;\
- there was a significant difference between **the male athletes age compared with female athletes age**;\
- there was a significant difference between **the number of male and female participants in teams at the Winter Olympics**;\
- there was a significant difference between **the the height of female athletes participating in the winter or summer games**.


**Teams and Medals**:\
- there is a **strong positive correlation** between the number of team members and the number of their losses;\
- there is a **marked degree of positive correlation** between the number of team members and the number of team medals:\
    - there is a marked degree of positive correlation between the number of team members and the number of team Gold medals;\
    - there is a marked degree of positive correlation between the number of team members and the number of team Silver medals;\
    - there is a marked degree of positive correlation between the number of team members and the number of team Bronze medals;
